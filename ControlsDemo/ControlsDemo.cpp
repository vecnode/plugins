...
ControlsDemo::ControlsDemo(const InstanceInfo& info)
...
//    };
//
//    pGraphics->AttachControl(new IWebViewControl(b.GetCentredInside(200), false, readyFunc, msgFunc, "C:\\Users\\oli\\Dev\\iPlug2\\Examples\\ControlsDemo\\WebView2Loader.dll", "C:\\Users\\oli\\Dev\\iPlug2\\Examples\\ControlsDemo\\"));
...
#pragma mark - About Box
    ...
    pGraphics->AttachControl(new IAboutBoxControl(b,
            ...
            [](IContainerBase* pParent, const IRECT& r) {
              pParent->AddChildControl(new ITextControl(IRECT(), "ControlsDemo Example", {DEFAULT_TEXT_SIZE * 5, COLOR_WHITE}));
...
#if IPLUG_EDITOR
void ControlsDemo::FlashBlueLED()
{
  GetUI()->GetControlWithTag(kCtrlTagBlueLED)->As<ILEDControl>()->TriggerWithDecay(1000);
}

void ControlsDemo::OnMidiMsgUI(const IMidiMsg& msg)
{
  if(GetUI())
  {
...
void ControlsDemo::OnReset()
{
  mPeakAvgMeterSender.Reset(GetSampleRate());
}

void ControlsDemo::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{
